...
@interface LeftSlideViewController ()<UIGestureRecognizerDelegate, MainPageViewControllerScrolled> {
    CGFloat _scalef;  //实时横向位移
}
@property (nonatomic,strong) UITableView	*leftTableview;			// 侧滑控制器
@property (nonatomic,assign) CGFloat		leftTableviewW;			// 侧滑 tableView
@property (nonatomic,strong) UIView			*contentView;			// 蒙版
@property (nonatomic,assign) BOOL			openLift;				//
@end
...
- (instancetype)initWithLeftView:(UIViewController *)leftVC
    ...
    if(self = [super init]){
		...
		MainPageViewController *mainVC = (MainPageViewController *)[nav topViewController];
...
- (void)handlePan: (UIPanGestureRecognizer *)rec{
	...
	CGPoint startPoint	= [rec locationInView:self.view];						//开始点
    ...
    //根据视图位置判断是左滑还是右边滑动
    if (needMoveWithTap && (rec.view.frame.origin.x >= 0)
		&& (rec.view.frame.origin.x <= (kScreenWidth - kMainPageDistance))) {
...
-(void)handeTap:(UITapGestureRecognizer *)tap{
    if ((!self.closed) && (tap.state == UIGestureRecognizerStateEnded)) {
        ...
        self.closed = YES;
        self.leftTableview.center		= CGPointMake(kLeftCenterX, kScreenHeight * 0.5);
        self.leftTableview.transform	= CGAffineTransformScale(CGAffineTransformIdentity,kLeftScale,1.0);
